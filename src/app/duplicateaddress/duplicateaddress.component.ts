...
@Component({
  selector: 'app-duplicateaddress',
  templateUrl: './duplicateaddress.component.html',
  styleUrls: ['./duplicateaddress.component.css']
})
export class DuplicateaddressComponent {
  ...
  myForm!: FormGroup;
  idArr: (any | undefined)[] = [];
  mapArr: (any)[] = []; // used id's to remove from inMemory hashmap
  ...
  getAllContact() {
    this.spinner.show();
    this.contactService.getAllContactsAddress().subscribe(data => {
      console.log(data);
      this.spinner.hide();
      this.archiveContactList = [];
      this.rootList = data.sort(function (a, b) {
        if (a.address.address1 < b.address.address1) {
          return -1;
        }
        if (a.address.address1 > b.address.address1) {
          return 1;
  ...
  onChangeArchive(contact: Root, target: EventTarget | null) {
    ...
    if (input.checked) {
      ...
      this.rootList.map(x => {
        if (x.sortingParameter == contact.sortingParameter) {
